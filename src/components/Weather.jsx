...
const Weather = () => {
    ...
    return (
        <div className='p-0'>
            ...
            >
                <div className='flex flex-col items-center justify-center p-4 text-slate-50
                h-full w-full bg-white-500 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-70 border border-gray-100'>
                    <input type='text'
                        placeholder='Enter a city'
                        value={city}
                        onChange={e => setCity(e.target.value)}
                        className='bg-slate-50 rounded-4xl text-slate-800 px-2 border-2 border-amber-200' />
                    <button
                        onClick={fetchWeather}
                        className=' text-slate-50 bg-amber-200 m-4 rounded-2xl px-3 py-1 transition-transform duration-300 ease-out hover:scale-110 border-4 border-slate-300'>Search</button>
